...
exports.startServer = (port, path, callback) => {
    require('dns').lookup(require('os').hostname(), function (err, add, fam) {
        ...
        app.get('/controller', function (req, res) {
            cons.handlebars(__dirname + '/views/controller.html', { socketUrl:localIPUrl })
                .then(function (html) {
                    res.send(html);
                })
                .catch(function (err) {
        ...
        app.get('/', function (req, res) {
            cons.handlebars(__dirname + '/views/index.html', { localIPUrl })
                .then(function (html) {
                    res.send(html);
                })
                .catch(function (err) {
        ...
        new Promise(function (resolve, reject) {
            ngrok.connect(Object.assign({
                addr:HTTP_PORT,
                key: __dirname + private.ssl.key,
                crt: __dirname + private.ssl.cert
            },private.ngrok), function (err, url) {
                if (err) {
                    console.error("## NGROK : Error gate : ",err.msg);
                    if(err.details && err.details.err)console.error("## NGROK : details : \n",err.details.err);
                    reject(err);
                }
                if (url) {
